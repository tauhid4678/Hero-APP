...
const Appcards = ({ app }) => {
  ...
  return (
    <div
      className={`card border-2 border-solid border-gray-50 bg-base-100 w-60 h-60 shadow-sm rounded-xl p-5 cursor-pointer transform transition-all duration-300
        ${details ? "h-auto" : "h-50"} hover:scale-105 hover:shadow-lg`}
    ...
    >
      ...
      <div className="card-body py-3 px-1">
        ...
        {details && (
          <div className="mt-2 p-2 border-t border-gray-200 text-sm text-gray-700">
            <p>{description}</p>
          </div>
        )}
      </div>
